...
function showApp(){$('loginView').classList.add('hidden');$('appView').classList.remove('hidden');$('userBadge').textContent=`${me.name||me.user_id||''} · ${isAdmin()?'관리자':'일반 사용자'}`;$('salesNav').classList.toggle('hidden',!(isAdmin()&&can('can_view_staff_sales')));$('usersNav').classList.toggle('hidden',!(isAdmin()&&can('can_manage_staff_users')));$('dbImportBtn')?.classList.toggle('hidden',!(isAdmin()&&can('can_import_staff_sites')));$('allSitesExportBtn')?.classList.toggle('hidden',!canExportAllSites());$('siteCreateBtn')?.classList.toggle('hidden',!canCreateSite());showPage(can('can_view_staff_sites')?'search':can('can_view_staff_sales')?'sales':'users');refreshDbStatus();if(can('can_view_staff_sites'))searchSites()}
...
async function exportAllSites(){
 ...
 try{
  ...
  for(const r of rows){
    ...
    aoa.push([...vals,formatPhone(r.client_phone||''),String(r.client_email||''),String(r.site_address||''),Number(r.excel_row)<0?'직접등록':'Excel/DB',r.id,String(r.source_file||'')]);
  }
