...
function editableSiteFieldsInOrder(){
 ...
 for(const g of siteEditGroups){
  for(const f of schema.fields.filter(x=>x.col>=g[1]&&x.col<=g[2]&&siteFieldAllowed(x)).sort((a,b)=>a.col-b.col)){
...
function moveSiteEditDataFieldNext(current){
 ...
 const nextGroup=siteEditGroups.findIndex(g=>next.col>=g[1]&&next.col<=g[2]);
...
async function ensureFullSiteRow(id,row=null){
 let r=row||lastSites.find(x=>Number(x.source_id)===Number(id));
 if(r&&Array.isArray(r.safe_values)&&r.safe_values.length>=137)return r;
 const{data,error}=await sb.from('staff_site_search').select('*').eq('source_id',Number(id)).maybeSingle();if(error)throw error;if(!data)return r||null;
 const ix=lastSites.findIndex(x=>Number(x.source_id)===Number(id));if(ix>=0)lastSites[ix]=data;else lastSites.push(data);
 return data;
}
...
async function importSalesExcel(file){
 if(!file)return;if(!canViewSales())return alert('매출 조회 권한이 없습니다.');
 try{
...
function userPermissionKeys(){return ['approved','can_use_staff_portal','can_view_staff_sites','can_create_staff_sites','can_edit_staff_sites','can_export_staff_sites','can_view_staff_sales','can_export_staff_sales','can_print_staff_sales','can_import_staff_sites','can_manage_staff_users']}
function syncUserRowDependencies(tr,changedKey=''){
 ...
 if(role==='admin'){
  ...
  if(['can_create_staff_sites','can_edit_staff_sites','can_export_staff_sites','can_import_staff_sites'].includes(changedKey)&&boxes[changedKey]?.checked&&boxes.can_view_staff_sites)boxes.can_view_staff_sites.checked=true;
  if(changedKey==='can_view_staff_sites'&&!boxes.can_view_staff_sites?.checked){['can_create_staff_sites','can_edit_staff_sites','can_export_staff_sites','can_import_staff_sites'].forEach(k=>{if(boxes[k])boxes[k].checked=false})}
  ...
  const anyFeature=['can_view_staff_sites','can_create_staff_sites','can_edit_staff_sites','can_export_staff_sites','can_view_staff_sales','can_export_staff_sales','can_print_staff_sales','can_import_staff_sites','can_manage_staff_users'].some(k=>boxes[k]?.checked);
  if(anyFeature&&boxes.can_use_staff_portal)boxes.can_use_staff_portal.checked=true;
 }
 const admin=role==='admin';
 tr.querySelectorAll('[data-k]').forEach(el=>{if(el.dataset.k==='role'||el.dataset.k==='approved')el.disabled=!isAdmin();else el.disabled=admin});
}
...
function renderUsers(rows){
 ...
 tb.innerHTML=(rows||[]).map(p=>{const admin=p.role==='admin';return `<tr data-user-row="${p.id}"><td>${esc(p.name||'')}</td><td>${esc(p.user_id||'')}</td><td>${esc(p.phone||'')}</td><td><select data-u="${p.id}" data-k="role" ${!isAdmin()?'disabled':''}><option value="viewer" ${!admin?'selected':''}>일반</option><option value="admin" ${admin?'selected':''}>관리자</option></select></td>${perms.map(k=>`<td class="permCell"><input type="checkbox" data-u="${p.id}" data-k="${k}" ${p[k]?'checked':''} ${(admin&&k!=='approved')||(!isAdmin()&&k==='approved')?'disabled':''}></td>`).join('')}<td><div class="userActions"><button class="primary userSaveBtn" data-save-user="${p.id}" disabled>저장됨</button>${isAdmin()?`<button data-reset="${p.id}">PW</button>${p.id!==me.id?`<button data-del="${p.id}">삭제</button>`:''}`:''}</div></td></tr>`}).join('');
 ...
 tb.querySelectorAll('[data-save-user]').forEach(b=>b.onclick=()=>saveUserPermissions(b.dataset.saveUser));
 tb.querySelectorAll('[data-reset]').forEach(b=>b.onclick=()=>resetPw(b.dataset.reset));
 tb.querySelectorAll('[data-del]').forEach(b=>b.onclick=()=>deleteUser(b.dataset.del));
 scheduleTableColumnResize($('userTable'),'users');
}
...
async function deleteUser(id){if(!confirm('이 직원 계정을 삭제할까요?'))return;try{await invokeAdmin({action:'delete',user_uuid:id});invalidateDataCaches('users');loadUsers(true)}catch(e){alert(e.message)}}
function syncRoleForm(){const admin=$('empRole').value==='admin';['permPortal','permSites','permSiteCreate','permSiteEdit','permAllSitesExport','permSales','permSalesExport','permSalesPrint','permImport','permUsers'].forEach(id=>{if(admin)$(id).checked=true;$(id).disabled=admin})}
