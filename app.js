...
function showApp(){$('loginView').classList.add('hidden');$('appView').classList.remove('hidden');$('userBadge').textContent=`${me.name||me.user_id||''} · ${isAdmin()?'관리자':'일반 사용자'}`;$('unwrittenNav')?.classList.toggle('hidden',!isAdmin());$('salesNav').classList.toggle('hidden',!(isAdmin()&&can('can_view_staff_sales')));$('usersNav').classList.toggle('hidden',!(isAdmin()&&can('can_manage_staff_users')));$('dbImportBtn')?.classList.toggle('hidden',!(isAdmin()&&can('can_import_staff_sites')));$('dbDeleteAllBtn')?.classList.toggle('hidden',!isAdmin());$('allSitesExportBtn')?.classList.toggle('hidden',!canExportAllSites());$('siteCreateBtn')?.classList.toggle('hidden',!canCreateSite());showPage(savedPage(),false);refreshDbStatus()}
...
function showPage(name,save=true){
 ...
 if(name==='sales'&&!(isAdmin()&&can('can_view_staff_sales')))return alert('매출 관리는 관리자만 사용할 수 있습니다.');
 if(name==='users'&&!(isAdmin()&&can('can_manage_staff_users')))return alert('사용자 관리는 관리자만 사용할 수 있습니다.');
