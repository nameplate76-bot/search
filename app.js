...
function showApp(){$('loginView').classList.add('hidden');$('appView').classList.remove('hidden');$('userBadge').textContent=`${me.name||me.user_id||''} · ${isAdmin()?'관리자':'일반 사용자'}`;$('salesNav').classList.toggle('hidden',!(isAdmin()&&can('can_view_staff_sales')));$('usersNav').classList.toggle('hidden',!(isAdmin()&&can('can_manage_staff_users')));$('dbImportBtn')?.classList.toggle('hidden',!(isAdmin()&&can('can_import_staff_sites')));$('siteCreateBtn')?.classList.toggle('hidden',!canCreateSite());showPage(can('can_view_staff_sites')?'search':can('can_view_staff_sales')?'sales':'users');refreshDbStatus();if(can('can_view_staff_sites'))searchSites()}
...
function renderSiteEditFields(){const g=siteEditGroups[siteEditGroupIndex]||siteEditGroups[0];const fields=schema.fields.filter(f=>f.col>=g[1]&&f.col<=g[2]&&siteFieldAllowed(f));$('siteEditFields').innerHTML=fields.map(f=>{const v=siteEditValues[f.col-1]??'',req=f.col===4?' required':'',fin=f.financial?' financialField':'';return `<label class="siteField${fin}"><span>${esc(f.label)}${f.col===4?' *':''}</span><input data-site-col="${f.col}" type="${siteInputType(f.col)}" value="${esc(v)}"${req}></label>`}).join('')||'<p class="hint">이 탭에서 입력할 수 있는 항목이 없습니다.</p>';$('siteEditFields').querySelectorAll('[data-site-col]').forEach(inp=>inp.oninput=()=>{siteEditValues[Number(inp.dataset.siteCol)-1]=inp.value})}
...
function syncRoleForm(){const admin=$('empRole').value==='admin';['permPortal','permSites','permSiteCreate','permSiteEdit'].forEach(id=>{$(id).disabled=false;if(admin)$(id).checked=true});['permSales','permSalesExport','permSalesPrint','permImport','permUsers'].forEach(id=>{$(id).checked=admin;$(id).disabled=admin})}
...
$('permSiteCreate').onchange=$('permSiteEdit').onchange=e=>{if(e.target.checked)$('permSites').checked=true};$('permSites').onchange=e=>{if(!e.target.checked){$('permSiteCreate').checked=false;$('permSiteEdit').checked=false}};$('siteCreateBtn').onclick=openCreateSite;$('siteEditClose').onclick=()=>$('siteEditDlg').close();$('siteEditCancel').onclick=()=>$('siteEditDlg').close();$('siteEditForm').onsubmit=saveSiteEdit;$('siteAddressSearchBtn').onclick=searchSiteAddress;$('siteAddressQuery').onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();searchSiteAddress()}};$('siteManualAddressBtn').onclick=()=>setSiteAddressMode('manual');$('siteSearchAddressModeBtn').onclick=()=>setSiteAddressMode('search');$('siteFormPhone').oninput=e=>{e.target.value=formatPhone(e.target.value)};
